  ...
  void slide2(uint8_t numReps,
    ...
    if(_slideRepCount < numReps){
      ...
      if(_slideNoteCount == 0){
        ...
        uint16_t currFreq = round(float(s1StartNote) + slideSlope*float(_noteTimer.get_time()));
        ...
        uint16_t currFreq = round(float(s2StartNote) + slideSlope*float(_noteTimer.get_time()));
